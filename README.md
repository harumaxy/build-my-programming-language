...
就活ポートフォリオ用のオリジナルプログラミング言語実装プロジェクト
...
1. **デモのしやすさ**: ブラウザ上でプレイグラウンドを作成可能。採用担当者がすぐに試せる
2. **型システム**: Union型やDiscriminated Unionを使ったAST定義が直感的
3. **エコシステム**: npm公開やWebアプリ化が容易
4. **chevrotain**: 高速で実績のあるTypeScript製パーサーライブラリ
...
- **パーサーコンビネータ**: 小さなパーサーを組み合わせる（chevrotainが採用）
...
- [ ] プロジェクトセットアップ（TypeScript, chevrotain, Vitest）
- [ ] Lexer実装（基本トークン）
- [ ] Parser実装（式、変数宣言）
- [ ] AST定義
- [ ] 基本的なEvaluator
...
- AST: プログラムの構造を表すデータ
- 意味解析: スコープ解決、未定義変数チェック等
- Evaluator: ASTを辿って実際に計算を実行
- ランタイム: 環境、組み込み関数、標準機能
