...
class MyLanguageParser extends EmbeddedActionsParser {
  ...
  // fn name(params) { }
  private functionDeclaration = this.RULE("functionDeclaration", (): Statement => {
  ...
  // 代入: x = expr
  private assignmentExpression = this.RULE("assignmentExpression", (): Expression => {
  ...
  // == !=
  private equalityExpression = this.RULE("equalityExpression", (): Expression => {
    let left = this.SUBRULE(this.comparisonExpression);
    this.MANY(() => {
      const operator = this.OR([
        { ALT: () => { this.CONSUME(EqualEqual); return "==" as BinaryOperator; } },
        { ALT: () => { this.CONSUME(NotEqual); return "!=" as BinaryOperator; } },
      ]);
  ...
  // < > <= >=
  private comparisonExpression = this.RULE("comparisonExpression", (): Expression => {
    let left = this.SUBRULE(this.additiveExpression);
    this.MANY(() => {
      const operator = this.OR([
        { ALT: () => { this.CONSUME(Less); return "<" as BinaryOperator; } },
        { ALT: () => { this.CONSUME(Greater); return ">" as BinaryOperator; } },
        { ALT: () => { this.CONSUME(LessEqual); return "<=" as BinaryOperator; } },
        { ALT: () => { this.CONSUME(GreaterEqual); return ">=" as BinaryOperator; } },
      ]);
  ...
  // + -
  private additiveExpression = this.RULE("additiveExpression", (): Expression => {
    let left = this.SUBRULE(this.multiplicativeExpression);
    this.MANY(() => {
      const operator = this.OR([
        { ALT: () => { this.CONSUME(Plus); return "+" as BinaryOperator; } },
        { ALT: () => { this.CONSUME(Minus); return "-" as BinaryOperator; } },
      ]);
  ...
  // * / %
  private multiplicativeExpression = this.RULE("multiplicativeExpression", (): Expression => {
    let left = this.SUBRULE(this.unaryExpression);
    this.MANY(() => {
      const operator = this.OR([
        { ALT: () => { this.CONSUME(Multiply); return "*" as BinaryOperator; } },
        { ALT: () => { this.CONSUME(Divide); return "/" as BinaryOperator; } },
        { ALT: () => { this.CONSUME(Modulo); return "%" as BinaryOperator; } },
      ]);
    ...
    return this.OR([
      {
        ALT: () => {
          ...
          return { type: "UnaryExpression" as const, operator: "-" as const, argument };
        },
      },
      {
        ALT: () => {
  ...
  private callMemberExpression = this.RULE("callMemberExpression", (): Expression => {
      ...
      expr = this.OR([
        {
          ALT: () => {
            ...
            this.CONSUME(RParen);
            return { type: "CallExpression" as const, callee: expr, arguments: args };
          },
        },
        {
          ALT: () => {
    ...
    return this.OR([
      // リテラル
      {
        ALT: () => {
          ...
          return { type: "NumberLiteral" as const, value: parseFloat(token.image) };
        },
      },
      {
        ALT: () => {
